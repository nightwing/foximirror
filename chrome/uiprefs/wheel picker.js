...
onReady=function(){
	t=Date.now()
	b=new ColourWheel(256)
	var body=document.documentElement
	body.appendChild(b.div)	
	
	body=document.getElementById('leftgrid').lastChild
	fproto=document.createElement('row')
	fproto.appendChild(document.createElement('label'))
	fproto.appendChild(t1=document.createElement('textbox'))
	t1.setAttribute('onclick',"start(this)")
	fproto.appendChild(t1=document.createElement('checkbox'))
	t1.setAttribute('onclick',"clearPref(this.prefname);updateFields()")
	
	for(var i in colorPrefs){
		f=body.appendChild(fproto.cloneNode(true))
		var col=colorPrefs[i]
		//f.style.display='inline'
		f=f.firstChild
		f.value=col
		f=f.nextSibling;
		f.prefname=col;f.value=getPref(col)
		f.nextSibling.prefname=col
	}
	body=document.getElementById('leftgrid').lastChild
	fproto=document.createElement('row')
	fproto.appendChild(document.createElement('label'))
	...
	t1.setAttribute('oninput',"savePref(this.prefname,this.value,'int');updateFields()")
	t1.setAttribute('onchange',"savePref(this.prefname,this.value,'int');updateFields()")
	t1.setAttribute('oncommand',"savePref(this.prefname,this.value,'int');updateFields()")
	t1.setAttribute('onkeypress',"savePref(this.prefname,this.value,'int');updateFields()")

	fproto.appendChild(t1=document.createElement('checkbox'))
	t1.setAttribute('onclick',"clearPref(this.prefname);updateFields()")
	
	for(var i in numerPrefs){
		f=body.appendChild(fproto.cloneNode(true))
		var col=numerPrefs[i]
		//f.style.display='inline'
		f=f.firstChild
		f.value=col
		f=f.nextSibling;
	...
	start=function(el){
		b.onUpdate=function(d){
			if(this.el&&this.el.style){
				this.el.value=d
				this.el.style.background=d
			}
			dump(this.el.prefname,d)
				savePref(this.el.prefname,d,'string')
			try{}catch(e){}
			updateFields()
			//dump(this,b.el.value,b.el)
		}
		b.el=el
