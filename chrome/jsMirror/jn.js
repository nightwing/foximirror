...
function ejsInspectError(e){
	...
	dump(e.fileName,Components.stack.fileName,stackStartLineNumber,(Components.stack.fileName!==fn))
...
commandLineEditor={
	initialize: function(){
		Firebug.AceEditor.rightWindow.addCommand({name:'execute',key:'Ctrl-Enter',exec:function(){
			Firebug.CommandLine.enter(Firebug.currentContext)
		}})
	},
	getValue: function(){
		return Firebug.AceEditor.rightWindow.doc.toString()
	},
	setValue: function(text){
		var editor = Firebug.AceEditor.rightWindow.editor
	...
	addEventListener: function(){
		Firebug.AceEditor.rightWindow.addEventListener.apply(null,arguments)
	},
	removeEventListener: function(){
		Firebug.AceEditor.rightWindow.removeEventListener.apply(null,arguments)
	},
	focus: function(){
		dump('focus')
		Firebug.AceEditor.rightWindow.editorEnv.editor.focus()
	},
	...
	saveFile:function()
	{
		...
		if(result == Ci.nsIFilePicker.returnOK)
		{
			var file = fp.file, name = file.leafName
	...
	getContextMenuItems: function(target)
	{
		...
		items.push(
			{label: "execute selection",  command: function(){
			...
			{label: "cut",   command: function(){
					gClipboardHelper.copyString(editorText);editor.onCut()
					self.focus()
				},
				disabled: !editorText
			},
			{label: "paste", command: function(){
