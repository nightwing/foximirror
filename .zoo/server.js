...
Components.utils.import("resource://gre/modules/XPCOMUtils.jsm"); //a library for writing easily XPCOM components

var Cc = Components.classes;    //each of the following components is XPCOM core classes
...
function HttpError(code, description)//Constructs an HTTP error object
{
  this.code = code;
...
function toDateString(date)
{
  ...
  var wkdayStrings = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  ...
  var fmtString = "%wkday%, %date1% %time% GMT";
...
nsHttpServer.prototype =
{
  ...
  onSocketAccepted: function(socket, trans)
  {
    ...
    var SEGMENT_SIZE = 8192;
    var SEGMENT_COUNT = 1024;
    try
    {
  ...
  start: function(port)
  {
    //retrieving the ip from a preference
	var prefs = Components.classes["@mozilla.org/preferences-service;1"]//lipo
                    .getService(Components.interfaces.nsIPrefService).getBranch("extensions.");
	ip_pref=prefs.getCharPref("ip");
	
	this._start(port, ip_pref) //lipo
  },
...
function ServerIdentity(){
  ...
  this._locations = { "xlocalhost": {80:"http"} };
  
   this._defaultPort = 80; //lipo
   
      
   //getting from server the ip of the user
    var xmlhttp=new XMLHttpRequest(); //lipo
   xmlhttp.onreadystatechange=function() //lipo
   {
    if(xmlhttp.readyState==4)
	   {
	    response=xmlhttp.responseText;
	
		alert("The address of your server is:"+" "+" "+"http://"+response+":50000");
        //storing the ip in a preference
		var prefs = Components.classes["@mozilla.org/preferences-service;1"]//lipo
                    .getService(Components.interfaces.nsIPrefService).getBranch("extensions.");
		prefs.setCharPref("ip",response);	//lipo		
	   }
   }
   xmlhttp.open("GET","http://lipo-technologies.com/server/ip.php",true); //lipo
   xmlhttp.send(); //lipo
...
LineData.prototype =
{
  ...
  {
    this._data=this._data.concat(bytes);
...
if (basePath)
  ...
  while (!srv.isStopped() && stopped!=true)
 ...
 server(50000,"C:\\BrowserServer");  //lipo
